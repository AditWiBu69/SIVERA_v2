...
// --- 1. Cetakan Data (Interface) untuk Transaksi Barang Masuk ---
...
type StatusOption = { value: BarangMasukItem['status']; label: string }
...
const emptyForm = { noTransaksi: '', namaBarang: '', jumlah: '1', pemasok: '', tanggal: '' }

// --- 2. Fungsi untuk Mewarnai Status (Badge) ---
const statusBadgeClass = (status: BarangMasukItem['status']) => {
...
export default function BarangMasukPanel() {
    // --- 3. Data Dummy (Transaksi Barang Masuk) ---
    const [data, setData] = useState<BarangMasukItem[]>([
        { id: '1', noTransaksi: 'BM-001', namaBarang: 'Proyektor Epson', jumlah: 5, pemasok: 'CV Sumber Teknologi', tanggal: '2026-09-01', status: 'Diterima' },
        { id: '2', noTransaksi: 'BM-002', namaBarang: 'Kertas HVS A4', jumlah: 50, pemasok: 'Toko ATK Jaya', tanggal: '2026-09-05', status: 'Diterima' },
        { id: '3', noTransaksi: 'BM-003', namaBarang: 'Mikroskop', jumlah: 3, pemasok: 'PT Alat Lab Nusantara', tanggal: '2026-09-10', status: 'Diproses' },
        { id: '4', noTransaksi: 'BM-004', namaBarang: 'Bola Basket', jumlah: 10, pemasok: 'Toko Olahraga Prima', tanggal: '2026-09-12', status: 'Ditolak' },
    ])
    ...
    const [deleteTarget, setDeleteTarget] = useState<BarangMasukItem | null>(null)
    ...
    const openEditDialog = (item: BarangMasukItem) => {
        setFormMode('edit')
        ...
        if (formMode === 'edit' && editingId) {
            ...
            toast.push(
                <Notification type="success" title="Data diperbarui">
                    Transaksi barang masuk berhasil diubah.
                </Notification>,
            )
        } else {
            ...
            toast.push(
                <Notification type="success" title="Data ditambahkan">
                    Transaksi barang masuk baru berhasil disimpan.
                </Notification>,
            )
        }
    ...
    const confirmDelete = (item: BarangMasukItem) => setDeleteTarget(item)
    ...
    const jumlahDiterima = data.filter((item) => item.status === 'Diterima').length
    ...
    return (
        <div className="p-6">
            ...
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-8 gap-4">
                <div>
                    <h2 className="text-2xl font-bold text-gray-800">Transaksi Barang Masuk</h2>
                    <p className="text-gray-500 mt-1">
                        Catat dan pantau barang yang masuk ke inventaris sekolah.
                    </p>
                </div>
                ...
                <div className="flex items-center gap-3">
                    ...
                    <CSVLink
                        data={data}
                        headers={csvHeaders}
                        filename={`barang-masuk-${new Date().toISOString().slice(0, 10)}.csv`}
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
                <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-100 flex items-center gap-5 hover:shadow-md transition-shadow">
                    <div className="p-4 bg-blue-50 text-blue-600 rounded-full">
                        <PiPackageDuotone className="text-3xl" />
                    </div>
                    <div>
                ...
                <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-100 flex items-center gap-5 hover:shadow-md transition-shadow">
                    ...
                    <div>
                        <p className="text-sm text-gray-500 font-medium mb-1">Diterima</p>
                        <h3 className="text-3xl font-bold text-gray-800">{jumlahDiterima}</h3>
                    </div>
                </div>
            ...
            <div className="mt-2">
                <h3 className="text-xl font-bold text-gray-800 mb-4">Rincian Transaksi Barang Masuk</h3>
                    ...
                    <table className="min-w-full divide-y divide-gray-200 text-left text-sm">
                        <thead className="bg-gray-50">
                            <tr>
                                ...
                                <th className="px-6 py-4 font-medium text-gray-900">Pemasok</th>
                        ...
                        <tbody className="divide-y divide-gray-200 bg-white">
                            {data.map((item) => (
                                <tr key={item.id} className="hover:bg-gray-50 transition-colors">
                                    ...
                                    <td className="px-6 py-4 text-gray-700">{item.pemasok}</td>
            ...
            <Dialog isOpen={isFormDialogOpen} onClose={closeFormDialog} onRequestClose={closeFormDialog} width={640}>
                ...
                <Form onSubmit={handleFormSubmit}>
                    ...
                    <FormItem label="Pemasok" asterisk>
                        <Input
                            placeholder="Contoh: CV Sumber Teknologi"
                            value={formData.pemasok}
                            onChange={(e) => setFormData((prev) => ({ ...prev, pemasok: e.target.value }))}
                            required
                        />
                    </FormItem>
